...
public class ParallelDDMinDelta {
	...
	// input: Arrays.asList("1_2", "2_3", "3_4", "1_3", "2_4", "1_4")
	// input: Arrays.asList("1_2", "2_3")
	// output: Arrays.asList("2_1", "3_2", "3_4", "1_3", "2_4", "1_4")
	// output: 2_1_3_4
	public String getFinalSeq(int seq_size, List<String> default_seq, List<String> delta_seq) {
		...
		for (int i = 0; i < seq.size(); i++) {
			seq_string = mergeSeq(seq_size, seq.get(i), seq);
			if (checkCircle(seq_string)) {
	...
	public String mergeSeq(int seq_size, String base_seq_string, List<String> seq) {
		if (base_seq_string.split("_").length > seq_size) {
			return base_seq_string;
		}
		String[] base_seq_array = base_seq_string.split("_");
		for (String s : seq) {
			String[] seq_array = s.split("_");
			if (base_seq_array[0].equals(seq_array[1])) {
				return mergeSeq(seq_size, seq_array[0] + "_" + base_seq_string, seq);
			}
			if (base_seq_array[base_seq_array.length - 1].equals(seq_array[0])) {
				return mergeSeq(seq_size, base_seq_string + "_" + seq_array[1], seq);
			}
		}
		return base_seq_string;
	}
